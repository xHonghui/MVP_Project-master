...
public class NetworkUtils {
    ...
    // 记录所有下载的请求，每次下载都会保存到这个集合中，如果下次下载该集合中存在相应的Observable，则直接使用，就不用再请求网络了
            ...
            synchronized (NetworkUtils.class) {
                if (apiService == null) {
                    Retrofit retrofit = new Retrofit.Builder()
                            .baseUrl(NetConfig.BASE_URL)
                            .client(getOkHttpClient())
                            .addConverterFactory(JsonConverterFactory.create())     // 添加自定义数据转换器
                            .addCallAdapterFactory(RxJavaCallAdapterFactory.create())
                            .build();
    ...
    public <T> Subscription executePost(final String url, final Map<String, String> parameters, final ResponseCallBack<T> callBack) {
        ...
        // getGenericInterfaces() 获取接口类型（可能存在实现多个接口的现象，所以是数组类型）
        ...
        return mNetService.postRequest(url, parameters)
                .compose(schedulersTransformer)     // 线程切换
                .subscribe(new NetSubscriber(finalNeedType, callBack));     // 发布
    }
